      ...
      >
        <Box direction="Column" gap="100">
          ...
          <Text as="label" size="L400" style={{ paddingTop: config.space.S400 }}>
            {t('Components.RegistrationTokenStage.registration_token', 'Registration Token')}
          </Text>
          <Input
        ...
        <Button variant="Critical" type="submit">
          <Text as="span" size="B400">
            {t('Components.RegistrationTokenStage.retry', 'Retry')}
          </Text>
        </Button>
        <Button variant="Critical" fill="None" outlined type="button" onClick={onCancel}>
          <Text as="span" size="B400">
            {t('Components.RegistrationTokenStage.cancel', 'Cancel')}
          </Text>
        </Button>
      </Box>
    </Dialog>
...
export function RegistrationTokenStageDialog({
  ...
  if (errorCode) {
    return (
      <RegistrationTokenErrorDialog
        defaultToken={token}
        title={errorCode}
        message={error ?? t('Components.RegistrationTokenStage.invalid_token', 'Invalid registration token provided.')}
  ...
  if (!token) {
    return (
      <RegistrationTokenErrorDialog
        defaultToken={token}
        title={t('Components.RegistrationTokenStage.registration_token', 'Registration Token')}
        message={t('Components.RegistrationTokenStage.please_submit_token', 'Please submit registration token provided by you homeserver admin.')}
        onRetry={handleSubmit}
        onCancel={onCancel}
      />
    );
