...
export function CreateRoomAliasInput({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <Box shrink="No" direction="Column" gap="100">
      <Text size="L400">{t('Components.CreateRoomAliasInput.address_optional', 'Address (Optional)')}</Text>
      <Text size="T200" priority="300">
        {t('Components.CreateRoomAliasInput.pick_unique_address', 'Pick an unique address to make it discoverable.')}
      </Text>
      <Input
        ref={aliasInputRef}
      ...
      {aliasAvailable === false && (
        <Box style={{ color: color.Critical.Main }} alignItems="Center" gap="100">
          ...
          <Text size="T200">
            <b>{t('Components.CreateRoomAliasInput.address_taken', 'This address is already taken. Please select a different one.')}</b>
          </Text>
        </Box>
      )}
    </Box>
