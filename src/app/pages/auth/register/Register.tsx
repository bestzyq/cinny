  ...
  return (
    <Box direction="Column" gap="500">
      <Text size="H2" priority="400">
        {t('Pages.Register.register', 'Register')}
      </Text>
      {registerFlows.status === RegisterFlowStatus.RegistrationDisabled && !sso && (
        <Text style={{ color: color.Critical.Main }} size="T300">
          {t('Pages.Register.registration_disabled', 'Registration has been disabled on this homeserver.')}
        </Text>
      )}
      {registerFlows.status === RegisterFlowStatus.RateLimited && !sso && (
        <Text style={{ color: color.Critical.Main }} size="T300">
          {t('Pages.Register.rate_limited', 'You have been rate-limited! Please try after some time.')}
        </Text>
      )}
      {registerFlows.status === RegisterFlowStatus.InvalidRequest && !sso && (
        <Text style={{ color: color.Critical.Main }} size="T300">
          {t('Pages.Register.invalid_request', 'Invalid Request! Failed to get any registration options.')}
        </Text>
      )}
          ...
          >
            {(supportedFlows) =>
              supportedFlows.length === 0 ? (
                <Text style={{ color: color.Critical.Main }} size="T300">
                  {t('Pages.Register.not_support_registration', 'This application does not support registration on this homeserver.')}
                </Text>
              ) : (
      ...
      <Text align="Center">
        {t('Pages.Register.have_account', 'Already have an account?')} <Link to={getLoginPath(server)}>{t('Pages.Register.login', 'Login')}</Link>
      </Text>
    </Box>
  );
}
