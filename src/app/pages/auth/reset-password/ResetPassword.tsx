  ...
  return (
    <Box direction="Column" gap="500">
      <Text size="H2" priority="400">
        {t('Pages.ResetPassword.reset_password', 'Reset Password')}
      </Text>
      <PasswordResetForm defaultEmail={resetPasswordSearchParams.email} />
      <span data-spacing-node />

      <Text align="Center">
        {t('Pages.ResetPassword.remember_password', 'Remember your password?')} <Link to={getLoginPath(server)}>{t('Pages.ResetPassword.login', 'Login')}</Link>
      </Text>
    </Box>
  );
}