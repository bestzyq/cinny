        ...
        >
          <Menu>
            <Header size="300" style={{ padding: `0 ${config.space.S200}` }}>
              <Text size="L400">{t('Pages.PasswordLoginForm.hint', 'Hint')}</Text>
            </Header>
            <Box
            ...
            >
              <Text size="T300">
                <Text as="span" size="Inherit" priority="300">
                  {t('Pages.PasswordLoginForm.username_hint', 'Username:')}
                </Text>{' '}
                user123
              </Text>
              <Text size="T300">
                <Text as="span" size="Inherit" priority="300">
                  {t('Pages.PasswordLoginForm.matrix_id_hint', 'Matrix ID:')}
                </Text>
                {` @user123:${server}`}
              </Text>
              <Text size="T300">
                <Text as="span" size="Inherit" priority="300">
                  {t('Pages.PasswordLoginForm.email_hint', 'Email:')}
                </Text>
                {` user123@${server}`}
              </Text>
            </Box>
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Inherit" gap="400">
      <Box direction="Column" gap="100">
        <Text as="label" size="L400" priority="300">
          {t('Pages.PasswordLoginForm.username', 'Username')}
        </Text>
        <Input
        ...
        {loginState.status === AsyncStatus.Error && (
          <>
            {loginState.error.errcode === LoginError.ServerNotAllowed && (
              <FieldError message={t('Pages.PasswordLoginForm.server_not_allowed', 'Login with custom server not allowed by your client instance.')} />
            )}
            {loginState.error.errcode === LoginError.InvalidServer && (
              <FieldError message={t('Pages.PasswordLoginForm.failed_find_server', 'Failed to find your Matrix ID server.')} />
            )}
          </>
        )}
      </Box>
      <Box direction="Column" gap="100">
        <Text as="label" size="L400" priority="300">
          {t('Pages.PasswordLoginForm.password', 'Password')}
        </Text>
          ...
          {loginState.status === AsyncStatus.Error && (
            <>
              {loginState.error.errcode === LoginError.Forbidden && (
                <FieldError message={t('Pages.PasswordLoginForm.invalid_credentials', 'Invalid Username or Password.')} />
              )}
              {loginState.error.errcode === LoginError.UserDeactivated && (
                <FieldError message={t('Pages.PasswordLoginForm.account_deactivated', 'This account has been deactivated.')} />
              )}
              {loginState.error.errcode === LoginError.InvalidRequest && (
                <FieldError message={t('Pages.PasswordLoginForm.invalid_request', 'Failed to login. Part of your request data is invalid.')} />
              )}
              {loginState.error.errcode === LoginError.RateLimited && (
                <FieldError message={t('Pages.PasswordLoginForm.rate_limited', 'Failed to login. Your login request has been rate-limited by server, Please try after some time.')} />
              )}
              {loginState.error.errcode === LoginError.Unknown && (
                <FieldError message={t('Pages.PasswordLoginForm.unknown_error', 'Failed to login. Unknown reason.')} />
              )}
            </>
          )}
          <Box grow="Yes" shrink="No" justifyContent="End">
            <Text as="span" size="T200" priority="400" align="Right">
              <Link to={getResetPasswordPath(server)}>{t('Pages.PasswordLoginForm.forget_password', 'Forget Password?')}</Link>
            </Text>
          </Box>
        </Box>
      </Box>
      <Button type="submit" variant="Primary" size="500">
        <Text as="span" size="B500">
          {t('Pages.PasswordLoginForm.login_button', 'Login')}
        </Text>
      </Button>
