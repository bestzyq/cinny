      ...
      {!parsedFlows.password && !parsedFlows.sso && (
        <>
          <Text style={{ color: color.Critical.Main }}>
            {t('Pages.Login.no_support', `This client does not support login on \"${server}\" homeserver. Password and SSO based login method not found.`, { server })}
          </Text>
          <span data-spacing-node />
        </>
      )}
      <Text align="Center">
        {t('Pages.Login.no_account', 'Do not have an account?')}{' '}
        <Link to={getRegisterPath(server)}>
          {t('Pages.Login.register', 'Register')}
        </Link>
      </Text>
    </Box>
  );
